...
const CHANNEL_ID = 'UC6t1O76G0jYXOAoYCm153dA';
...
async function summarize({ title, transcript }) {
  const systemPrompt = 'Sei un esperto analista di podcast in lingua inglese. Produci sempre output HTML pulito (solo <h2>, <p>, <strong>, <ul>, <li>) senza tag <html>, <body> o <style>. Non aggiungere preamboli, commenti o note finali. Mai usare blocchi di codice triple-backtick.';
  const userPrompt = `Ti fornisco la trascrizione automatica in inglese di un episodio del Lenny's Podcast intitolato: ${title}

Devi produrre un singolo output HTML in italiano con esattamente questa struttura:

<h2>Riassunto</h2>
<p><strong>Argomento principale:</strong> una frase concisa che identifichi il tema centrale dell'episodio.</p>
<p><strong>Concetti chiave affrontati:</strong></p>
<ul>
  <li>Da 6 a 10 voci, ciascuna una frase compatta su un'idea distinta affrontata nell'intervista.</li>
</ul>
<p><strong>Affermazioni fondamentali:</strong></p>
<ul>
  <li>Da 5 a 8 tesi sostenute con forza dall'ospite, ciascuna formulata come affermazione completa.</li>
</ul>
...
${transcript}`;
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'HTTP-Referer': 'https://github.com/lennys-podcast-summarizer',
      'X-Title': "Lenny's Podcast Summarizer",
    },
    body: JSON.stringify({
...
<p style="color:#666;font-size:13px;margin:0 0 8px 0">Nuovo episodio &middot; Lenny's Podcast</p>
  ...
  for (const entry of newOnes.reverse()) {
    ...
    try {
      ...
      if (transcript.length < MIN_TRANSCRIPT_LEN) {
        ...
        await sendEmail({ subject: `Lenny's Podcast — ${entry.title}`, html });
